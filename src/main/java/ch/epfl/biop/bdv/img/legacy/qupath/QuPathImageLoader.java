 ...
 * are some limitations: only bioformats image server and rotated image server
 * are supported ( among probably other limitations ). Also, editing files in
 * the QuPath project after it has been converted to a xml bdv dataset is not
 * guaranteed to work.
 *
	...
	MultiResolutionImgLoader, Closeable
{
	...
	public QuPathImageLoader(URI quPathProject, BioFormatsBdvOpener openerModel,
		final AbstractSequenceDescription<?, ?, ?> sequenceDescription,
		int numFetcherThreads, int numPriorities)
	{
		...
		try {
			...
			project.images.forEach(image -> {
				...
				QuPathBioFormatsSourceIdentifier identifier =
					new QuPathBioFormatsSourceIdentifier();
				if (image.serverBuilder.builderType.equals("rotated")) {
					...
					if (angleDegreesStr.equals("NONE")) {
						...
						identifier.angleRotationZAxis = (Double.parseDouble(
							angleDegreesStr) / 180.0) * Math.PI;
					}
	...
	public BioFormatsSetupLoader<?, ?, ?> getSetupImgLoader(int setupId) {
		if (imgLoaders.containsKey(setupId)) {
			...
			try {
				imgL = new BioFormatsSetupLoader(opener, iS, iC, setupId, tTypeGetter
					.get(setupId), vTypeGetter.get(setupId), () -> cache);
			}
			catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static class QuPathEntryAndChannel {
		...
		public QuPathEntryAndChannel(QuPathBioFormatsSourceIdentifier entry,
			int iChannel)
		{
			this.entry = entry;
