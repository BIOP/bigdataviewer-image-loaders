...
public class QuPathToSpimData {
	...
	final Map<BioFormatsTools.BioformatsChannel, Integer> channelToId = new HashMap<>();

	final Map<URI, BioFormatsBdvOpener> openerMap = new HashMap<>();

	final Map<URI, Integer> uriToFileIndexMap = new HashMap<>();

	final Map<Integer, QuPathImageLoader.QuPathEntryAndChannel> viewSetupToQuPathEntryAndChannel =
		new HashMap<>();
		...
		final BioFormatsBdvOpener openerModel)
	{
		...
		try {
			...
			project.images.forEach(image -> {
				...
				QuPathImageLoader.QuPathBioFormatsSourceIdentifier identifier =
					new QuPathImageLoader.QuPathBioFormatsSourceIdentifier();
				...
				if (image.serverBuilder.builderType.equals("rotated")) {
					...
					if (angleDegreesStr.equals("NONE")) {
						...
						identifier.angleRotationZAxis = (Double.parseDouble(angleDegreesStr) /
							180.0) * Math.PI;
					}
						...
						"qupath.lib.images.servers.bioformats.BioFormatsServerBuilder"))
					{
						try {
							...
							if (!openerMap.containsKey(image.serverBuilder.uri)) {
								...
								uriToFileIndexMap.put(image.serverBuilder.uri,
									fileIndexCounter);
								fileIndexCounter++;
							}
							...
							identifier.entryID = project.images.get(
							...
							if (iSerie == -1) {
								logger.error(
									"Series not found in qupath project server builder!");
								identifier.bioformatsIndex = -1;
							}
							else {
								identifier.bioformatsIndex = Integer.parseInt(
									image.serverBuilder.args.get(iSerie + 1));
							}

							logger.debug(identifier.toString());
							//quPathSourceIdentifiers.add(identifier);

							BioFormatsBdvOpener opener = openerMap.get(
								image.serverBuilder.uri);
							...
							logger.debug("\t Serie " + identifier.bioformatsIndex +
								" Number of timesteps = " + omeMeta.getPixelsSizeT(
									identifier.bioformatsIndex).getNumberValue().intValue());
							// ---------- Serie > Channels
							logger.debug("\t Serie " + identifier.bioformatsIndex +
								" Number of channels = " + omeMeta.getChannelCount(
							...
							if (omeMeta.getPixelsSizeT(identifier.bioformatsIndex)
								.getNumberValue().intValue() > maxTimepoints)
							{
								maxTimepoints = omeMeta.getPixelsSizeT(
									identifier.bioformatsIndex).getNumberValue().intValue();
							}

							Dimensions dims = BioFormatsTools.getSeriesDimensions(omeMeta,
								identifier.bioformatsIndex); // number of pixels no	calibration
							logger.debug("X:" + dims.dimension(0) + " Y:" + dims.dimension(
								1) + " Z:" + dims.dimension(2));
							VoxelDimensions voxDims = BioFormatsTools
								.getSeriesVoxelDimensions(omeMeta, identifier.bioformatsIndex,
									opener.u, opener.voxSizeReferenceFrameLength);

							QuPathEntryEntity qpentry = new QuPathEntryEntity(
								identifier.entryID);
							qpentry.setName(QuPathEntryEntity.getNameFromURIAndSerie(
								identifier.uri, identifier.bioformatsIndex));
							qpentry.setQuPathProjectionLocation(Paths.get(quPathProject)
								.toString());
							channels.forEach(iCh -> {
								QuPathImageLoader.QuPathEntryAndChannel usc =
									new QuPathImageLoader.QuPathEntryAndChannel(identifier, iCh);
								viewSetupToQuPathEntryAndChannel.put(viewSetupCounter, usc);
								int ch_id = getChannelId(omeMeta, identifier.bioformatsIndex,
									iCh, memo.isRGB());

								String setupName = image.imageName + "_" + getChannelName(
									omeMeta, identifier.bioformatsIndex, iCh);
								...
								FileIndex fi = new FileIndex(uriToFileIndexMap.get(
									identifier.uri), identifier.sourceFile);
								vs.setAttribute(fi);
								...
								ARGBType color = BioFormatsTools.getColorFromMetadata(omeMeta,
									identifier.bioformatsIndex, iCh);

								ds.isSet = true;
								ds.color = new int[] { ARGBType.red(color.get()), ARGBType
									.green(color.get()), ARGBType.blue(color.get()), ARGBType
										.alpha(color.get()) };

								vs.setAttribute(ds);
								viewSetups.add(vs);
								logger.debug("View Setup " + viewSetupCounter + " series " +
									iSerie + " channel " + iCh);
								viewSetupCounter++;
							});
			...
			for (int iViewSetup = 0; iViewSetup < viewSetupCounter; iViewSetup++) {
				QuPathImageLoader.QuPathEntryAndChannel usc =
					viewSetupToQuPathEntryAndChannel.get(iViewSetup);
				...
				final int nTimepoints = omeMeta.getPixelsSizeT(bfIndex).getNumberValue()
					.intValue();
				final int vs = iViewSetup;

				AffineTransform3D affine = BioFormatsTools.getSeriesRootTransform(
					omeMeta, bfIndex, opener.u, opener.positionPreTransformMatrixArray, // AffineTransform3D
																																							// positionPreTransform,
					opener.positionPostTransformMatrixArray, // AffineTransform3D
																										// positionPostTransform,
					opener.positionReferenceFrameLength, opener.positionIsImageCenter, // boolean
																																							// positionIsImageCenter,
					opener.voxSizePreTransformMatrixArray, // voxSizePreTransform,
					opener.voxSizePostTransformMatrixArray, // AffineTransform3D
																									// voxSizePostTransform,
					opener.voxSizeReferenceFrameLength, // null, //Length
				...
				if (viewSetupToImageEntry.get(vs) != null) {
					if (viewSetupToImageEntry.get(vs).serverBuilder != null)
						if (viewSetupToImageEntry.get(vs).serverBuilder.metadata != null)
							pixelCalibrations = viewSetupToImageEntry.get(
								vs).serverBuilder.metadata.pixelCalibration;
				}
				...
				if (pixelCalibrations != null) {
					...
					logger.debug("ScaleX: " + scaleX + " scaleY:" + scaleY + " scaleZ:" +
						scaleZ);
					if ((Math.abs(scaleX - 1.0) > 0.0001) || (Math.abs(scaleY -
						1.0) > 0.0001) || (Math.abs(scaleZ - 1.0) > 0.0001))
					{
